...
OggVorbis_File * oggOpen(const CString & file, CString & error ) {
...
CString oggGetComments(const CString & file) {
	int i;
    char *decoded_value;
	CString out,error;

	OggVorbis_File * ogg = oggOpen(file, error);
	if (ogg == NULL)
		return out;
...
TEST(oggVComment, oggGetComments)
{
	CString file = "..\\testdata\\01.ogg";
	CString c = oggGetComments(file);
}
CString oggGetVal(OggVorbis_File * ogg, const CString & srcKey) {

	vorbis_comment *vc = ov_comment(ogg, -1);
	CString ucomment,out,key,value;
	int i;
	char *decoded_value;
	...
	for (i = 0; i < vc->comments; i++)
    {
	    if (utf8_decode(vc->user_comments[i], &decoded_value) >= 0)
        {
			ucomment = decoded_value;
            free(decoded_value);

			key = String::field(ucomment,"=",1);
			if (key.CompareNoCase(srcKey) == 0) {
				value = String::extract(ucomment,"=","");
				return value;
			}
        }
    }
...
TEST(oggVComment, oggGetVal)
{
	CString file = "..\\testdata\\01.ogg";
	CString error;
	OggVorbis_File * ogg = oggOpen(file, error);
	CString artist = oggGetVal(ogg, "Artist");
}

CString oggSetVal(OggVorbis_File * ogg, const CString & srcKey,
				  const CString & srcVal) {
	...
	for (i = 0; i < vc->comments; i++)
    {
	    if (utf8_decode(vc->user_comments[i], &decoded_value) >= 0)
        {
			...
			key = String::field(ucomment,"=",1);
			if (key.CompareNoCase(srcKey) != 0) {
				slist.AddTail(ucomment);
			}
        }
    }
	// Now slist contains all existing comments minus the srcKey one(s),
	// so now append the new key/val
	...
	// Now replace the entire comment block
