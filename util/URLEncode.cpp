...
CString CURLEncode::unconvert(char val[]) {
	CString csRet;
	csRet = hexToCh(val);
	return csRet;
}
...
// TO URL ENCODE FORM.
CString CURLEncode::URLEncode(const CString & pcsEncode)
{	
	int ichar_pos;
	CString csEncode;
	CString csEncoded;	
...
CString CURLEncode::URLDecode(const CString & pcsDecode) {
	int ichar_pos;
	CString csDecode;
	CString csDecoded;	
	int m_length;
//	int ascii_value;
	...
	for(ichar_pos = 0; ichar_pos < m_length; ichar_pos++)
	{
		...
		if ('%' == ch) {
			char hex[2];
			hex[0] = csDecode.GetAt(++ichar_pos);
			hex[1] = csDecode.GetAt(++ichar_pos);
...
TEST(CURLEncode,test)
{
	CURLEncode url;
	CString string("abc 123 !@#$%^&*()_+=\\|+_)(*&^%$#@!~`{}][';\":/.,<>?");
	string += "\r\n\t";
	CString encoded = url.URLEncode(string);
	CString decoded = url.URLDecode(encoded);
