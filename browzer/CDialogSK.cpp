...
public:
	BitmapToCRect(HBITMAP bm, CRect & rect, LayOutStyle los, 
		DWORD width, DWORD height, CString & desc): m_hBitmap(bm),
...
BOOL CDialogSK::applyOneBg(CDC* pDC, BitmapToCRect * bmcr, int offset)
{
    ...
    {
        pDC->BitBlt(bmcr->m_rect.left+offset, bmcr->m_rect.top+offset, bmcr->m_rect.Width(), bmcr->m_rect.Height(), &dc, 0, 0, SRCCOPY);
    }
    ...
    {
        pDC->StretchBlt(bmcr->m_rect.left+offset, bmcr->m_rect.top+offset, 
			bmcr->m_rect.Width(), bmcr->m_rect.Height(), &dc, 
			0, 0, bmcr->m_width, bmcr->m_height, SRCCOPY);
    }
