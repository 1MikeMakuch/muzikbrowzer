...
CButtonST::CButtonST(BOOL isbutton)
{
	logit=FALSE;
	m_bIsPressed		= FALSE;
	m_bIsFocused		= FALSE;
...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (m_bIsButton && (m_bIsFlat == FALSE || (m_bIsFlat && m_bDrawFlatFocus)))
	{
		// Draw the focus rect
		if (m_bIsFocused)
		{
...
//
void CButtonST::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, 
	CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, 
	CRect* rpImage)
{
//	bIsPressed = FALSE;
	CRect rBtn;
...
//	if (m_bIsButton && bIsPressed && m_bIsCheckBox == FALSE)
...
//		byIndex = (m_csBitmaps[1].hBitmap == NULL ? 0 : 1);
	...
	{
//		if (logit) logbmp(hdcMem, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight);
...
void CButtonST::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if (m_bIsButton && m_bIsPressed && m_bIsCheckBox == FALSE)
	...
	*/
	...
	{
		if (m_bIsButton && (m_bMouseOnButton || m_bIsPressed)) 
		{
...
//
DWORD CButtonST::SetBitmaps(LPCTSTR sBitmapIn, COLORREF crTransColorIn, 
							LPCTSTR sBitmapHover, COLORREF crTransColorHover, 
							LPCTSTR sBitmapOut, COLORREF crTransColorOut)
{
    HBITMAP    hBitmapIn	    = NULL;
	HBITMAP    hBitmapOut       = NULL;
	HBITMAP		hBitmapHover	= NULL;
    hBitmapIn = (HBITMAP)::LoadImage(0, sBitmapIn, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
	if (sBitmapOut != NULL) {
		hBitmapOut = (HBITMAP)::LoadImage(0, sBitmapOut, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
	}
	if (sBitmapHover != NULL) {
		hBitmapHover = (HBITMAP)::LoadImage(0, sBitmapHover, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
	}

    return SetBitmaps(hBitmapIn, crTransColorIn,
		hBitmapHover, crTransColorHover,
		hBitmapOut, crTransColorOut);
}
DWORD CButtonST::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, 
							int nBitmapHover, COLORREF crTransColorHover, 
							int nBitmapOut, COLORREF crTransColorOut)
{
	...
	hBitmapHover = (HBITMAP)::LoadImage(hInstResource, MAKEINTRESOURCE(nBitmapHover), IMAGE_BITMAP, 0, 0, 0);
...
DWORD CButtonST::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, 
							HBITMAP hBitmapHover, COLORREF crTransColorHover, 
...
//			}
		...
		if (hBitmapHover)
		{
			m_csBitmaps[2].hBitmap = hBitmapHover;
			m_csBitmaps[2].crTransparent = crTransColorHover;
			// Get bitmap size
			nRetValue = ::GetObject(hBitmapHover, sizeof(csBitmapSize), &csBitmapSize);
			if (nRetValue == 0)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
			...
			// Create mask for bitmap Hover
			m_csBitmaps[2].hMask = CreateBitmapMask(hBitmapHover, m_csBitmaps[2].dwWidth, m_csBitmaps[2].dwHeight, crTransColorHover);
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, CRect* pRect)
{
	...
	if (m_bIsButton && m_bIsFlat == FALSE)
	{
